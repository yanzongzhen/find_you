...
from spider.position_utils import *
import requests
import json
import datetime
...
class Location(object):
    ...
    def __init__(self, image_path):
        ...
        self.api_key = "f59ce697f33cdfd6ea80d133873eda74"

        self.url_get_position = 'https://restapi.amap.com/v3/geocode/regeo?key={}&location={}'

    def run(self):
        status, coordinate, take_time = self.__get_image_ability()

        if not status:
            if take_time:
                return coordinate, take_time
            else:
                return coordinate, ''
    ...
    def __get_image_ability(self):
        ...
        if img_exif:
                # 纬度数
            latitude_gps = img_exif.get('GPS GPSLatitude')

            # N,S 南北纬方向
            latitude_direction = img_exif.get('GPS GPSLatitudeRef')

            # 经度数
            longitude_gps = img_exif.get('GPS GPSLongitude')

            # E,W 东西经方向
            longitude_direction = img_exif.get('GPS GPSLongitudeRef')

            # 拍摄时间
            take_time = img_exif.get('EXIF DateTimeOriginal')
            ...
            if latitude_gps and longitude_gps and take_time:
                ...
                return True, f'{location[0]},{location[1]}', take_time
            else:
                print(f'获取的图片数据属性不完整')
                return False, '获取的图片数据属性不完整', ''
        else:
            print('抱歉，图片不是原图，没法获取到图片属性。')
            return False, '抱歉，图片不是原图，没法获取到图片属性。', ''
